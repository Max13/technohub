...
@section('content')
    <main class="container-hs text-center">
        ...
        <div class="mt-5">
            <p>{!! __('You need to be logged-in to access this network. Use your <u>Company</u> credentials to continue (usually, same as the part before the "@" of your e-mail address.') !!}</p>
        ...
        <form class="mt-4 text-start" method="POST" action="{{ route('auth.ldap.doLogin', request()->all(), false) }}">
            @csrf
            ...
            <button class="btn btn-primary w-100 @error('callback') disabled @enderror" type="submit">{{ __('Login') }}</button>
        </form>
